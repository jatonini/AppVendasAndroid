...
class MainApp(App):
    ...
    def on_start(self):
        ...
        #carregas as fotos dos produtos
    ...
    def carregar_infos_usuarios(self):
        try:
            ...
            requisicao = requests.get(f'https://app-de-vendas-curso-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}')
            ...
            homepage.ids['label_total_vendas'].text = f'[color=#FFFFFF]Total de Vendas:[/color] [b]R${total_vendas}[/b]'
    ...
    def mudar_foto_perfil(self, foto, *args):
        ...
        info = f'{{"avatar": "{foto}"}}'
        requisicao = requests.patch(f'https://app-de-vendas-curso-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}',
                                    data=info)
        self.mudar_tela('ajustespage')


    def adicionar_vendedor(self, id_vendedor_adicionado):
        link = f'https://app-de-vendas-curso-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"&equalTo="{id_vendedor_adicionado}"'
        ...
        if requisicao_dic == {}:
            ...
            if id_vendedor_adicionado in equipe:
                ...
                requests.patch(f'https://app-de-vendas-curso-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}',
                                 data = info)
                mensagem_texto.text = 'Vendedor adicionado com sucesso'
                # para adicionar um novo banner na lista de equipe
                pagina_listavendedores = self.root.ids['listarvendedorespage']
                lista_vendedores = pagina_listavendedores.ids['lista_vendedores']
    ...
    def selecionar_cliente(self, foto, *args):
        ...
        for item in list(lista_clientes.children):
            ...
            #pintar de azul as letras do icone selecionado
            try:
                texto = item.text
    ...
    def adicionar_venda(self):
        ...
        if cliente and produto and unidade and preco and quantidade and (type(preco) == float) and (type(quantidade) == float):
            ...
            requests.post(f'https://app-de-vendas-curso-default-rtdb.firebaseio.com/{self.local_id}/vendas.json?auth={self.id_token}',
                          data= info)
            ...
            requisicao = requests.get(f'https://app-de-vendas-curso-default-rtdb.firebaseio.com/{self.local_id}/total_vendas.json?auth={self.id_token}')
    ...
    def carregar_todas_vendas(self):
        ...
        requisicao = requests.get(f'https://app-de-vendas-curso-default-rtdb.firebaseio.com/.json?{self.local_id}.json?orderBy="id_vendedor"')
        requisicao_dic = requisicao.json()
        ...
        # preencher total de vendas
        pagina_todasvendas.ids['label_total_vendas'].text = f'[color=#FFFFFF]Total de Vendas:[/color] [b]R${total_vendas}[/b]'
    ...
    def logout(self, instance):
        # Limpar as credenciais de login
        with open('refreshtoken.txt', 'w') as arquivo:
            arquivo.write('')

        #limpando a foto de perfil do usuario
        foto_perfil = self.root.ids['foto_perfil']
        foto_perfil.source = f'icones/fotos_perfil/hash.png'
