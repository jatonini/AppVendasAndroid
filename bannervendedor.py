...
class BannerVendedor(FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        with self.canvas:
            Color(rgb=get_color_from_hex('#4F3E51'))
        ...
        link = f'https://app-de-vendas-curso-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"&equalTo="{id_vendedor}"'
        ...
        imagem = ImageButton(source = f"icones/fotos_perfil/{avatar}",
                             pos_hint= {"right": 0.28, "top": 0.9}, size_hint = (0.3, 0.8),
        ...
        label_total = LabelButton(text=f"Total de Vendas: R${total_vendas}",
                                  pos_hint= {"right": 0.6, "top":0.9}, size_hint = (0.5, 0.5),
                                  on_release = partial(meu_aplicativo.carregar_vendas_vendedor, valor))
